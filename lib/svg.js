...
// Strips <script> elements and event-handler/javascript: attributes so that
// malicious SVG markup cannot execute script in the rendered page.
function sanitize (element) {
  element.children = element.children.filter((child) => {
    ...
    if (child.name && child.name.toLowerCase() === 'script') {
      return false;
    }
  ...
  for (const attr of Object.keys(element.attrs)) {
    const value = String(element.attrs[attr]);

    if (/^on/i.test(attr) || (/href$/i.test(attr) && /^\s*javascript:/i.test(value))) {
...
function createSvgHelper (settings) {
  ...
  return function svg (...args) {
    ...
    return new Handlebars.SafeString(sanitize(parsed).root());
  }
}
